🇬🇧 English

App Name: Trendstore
Developer: Nezir Coşkunkan

You can delete your account directly in the Trendstore app by following this path:

My Account → Account Settings → Delete Account

If you are unable to access the app, you can request account deletion by sending an email to:
📧 trendstore685@gmail.com

Please include the email address associated with your account in your request.



🇹🇷 Türkçe

Uygulama Adı: Trendstore
Geliştirici: Nezir Coşkunkan

Trendstore uygulaması üzerinden hesabınızı silmek için aşağıdaki yolu izleyebilirsiniz:

Hesabım → Hesap Ayarları → Hesabı Sil

Uygulamaya erişiminiz yoksa, hesap silme talebinizi aşağıdaki e-posta adresine gönderebilirsiniz:
📧 trendstore685@gmail.com

Lütfen e-postanızda hesabınıza kayıtlı e-posta adresini belirtiniz.



🇷🇺 Русский

Название приложения: Trendstore
Разработчик: Nezir Coşkunkan

Вы можете удалить свой аккаунт в приложении Trendstore, следуя этому пути:

My Account → Account Settings → Delete Account

Если у вас нет доступа к приложению, вы можете запросить удаление аккаунта, отправив письмо на адрес:
📧 trendstore685@gmail.com

Пожалуйста, укажите адрес электронной почты, связанный с вашим аккаунтом.

